  ...
  <div v-loading="isLoading" element-loading-text="用户信息加载中...">
    <div class="background" v-if="showAccountManagement">
      ...
      <div class="container">
          ...
          <button v-if="!isEditMode" @click="enterEditMode" class="change">{{ isEditMode ? '取消修改' : '修改个人信息' }}</button>
          <button v-if="isEditMode" @click="saveChanges" class="change">保存修改</button>
      ...
      <input v-if="isEditMode" type="file" ref="fileInput" id="fileInput" @change="displayImage">
          ...
          <div class="editable-info">
            <div class="info">
              <label>账户: <span>{{ editedUsername || username }}</span></label>
...
export default {
  ...
  methods: {
    ...
    returnToSystem(){
      this.$router.push({name: 'totalCourse', query: {userId: this.userId, nowNav: 'check-courses'}})
    },
    async fetchUserInfo() {
      this.isLoading = true
      try {
